...
export default function TiendaAcceso({
  ...
  const [paso, setPaso] = useState<"cedula" | "clave" | "registro">("cedula");
  ...
  async function continuarCedula() {
    ...
    try {
      ...
      setPaso(est.registrado ? "clave" : "registro");
    } catch (e) {
  ...
  async function registrar() {
    ...
    setCargando(true); setError(null);
    try {
      onAutenticado(await registrarTrabajador({
        cedula: cedula.trim(), foto, clave, telefono: telefono.trim() || undefined,
      }));
    } catch (e) {
  ...
  // ── Paso 3: registro (primer ingreso) ───────────────────────────────────────
  return (
    <div>
      <button onClick={volver} className="mb-2 text-xs font-semibold text-brand-brown/50 hover:text-brand-amber">← Cambiar cédula</button>
      ...
      <p className="mt-1 text-xs font-medium text-brand-brown/55">
        Es tu primer ingreso. Verifica tu identidad con la foto de tu cédula y crea tu contraseña.
      </p>
      ...
      {foto ? (
        ...
        <button type="button" onClick={() => fotoRef.current?.click()}
          className="mt-4 flex h-24 w-full items-center justify-center gap-2 rounded-2xl border border-dashed border-brand-brown/30 text-sm font-medium text-brand-brown/55 transition hover:border-brand-amber/50 hover:text-brand-amber">
      ...
      <div className="mt-4 space-y-2">
        <input value={clave} onChange={(e) => setClave(e.target.value)} type={verClave ? "text" : "password"} placeholder="Crea tu contraseña" className={inputCls} />
        <input value={clave2} onChange={(e) => setClave2(e.target.value)} type={verClave ? "text" : "password"} placeholder="Repite la contraseña" className={inputCls} />
        ...
        <input value={telefono} onChange={(e) => setTelefono(e.target.value.replace(/\D/g, "").slice(0, 10))} inputMode="numeric" placeholder="Teléfono de contacto (opcional)" className={inputCls} />
      </div>

      <button onClick={registrar} disabled={cargando} className={`mt-4 ${btnCls}`}>
        {cargando ? "Verificando cédula…" : "Verificar y crear contraseña"}
      </button>
      {error && <p className="mt-3 text-sm font-medium text-red-600">{error}</p>}
    </div>
  );
